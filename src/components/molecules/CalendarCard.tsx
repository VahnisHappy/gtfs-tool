...
export default function CalendarCard({ calendar, isSelected, onSelect }: CalendarCardProps) {
    return (
        <li
            onClick={onSelect}
            className={`px-2 py-1 cursor-pointer transition-colors border-b border-gray-200 ${isSelected
                    ? 'bg-blue-50'
            ...
            <div className="space-y-1">
                ...
                <div className="flex gap-1">
                    {calendar.days.map((isActive, idx) => (
                        <div
                            key={idx}
                            className={`w-6 h-6 flex items-center justify-center text-xs rounded ${isActive
